...
from tkinter import Tk, PhotoImage, Menu, Frame, Text, Scrollbar, IntVar, \
    StringVar, END, Toplevel, Label, Entry, Checkbutton, Button
...
# messagebox for about, help & exit
def display_about_messagebox(event=None):
    tkinter.messagebox.showinfo("About", PROGRAM_NAME + "\nTKINTER Developed application\n GUI in Python")
def display_help_messagebox(event=None):
     tkinter.messagebox.showinfo("Help", "HELP: \nTKINTER Developed application\n GUI in Python ", icon='question')
def exit_editor(event=None):
     if tkinter.messagebox.askokcancel("Close the app?", "Are you sure you want to close the app? Dont forget to save!"):
          root.destroy()
# adding File->New, File->Open, File->Save, File->Save As
# implemenying functions new_file, open_file, save, save_as
...
def open_file(event=None):
     ...
     if input_file_name:
          ...
          root.title(f'{os.path.basename(file_name)} - {PROGRAM_NAME}')
          content_text.delete(1.0, END)
...
def save(event=None):
     ...
     if not file_name:
         ...
         write_to_file(file_name)
     return "break"
# adding functionalities to the text editor
# implementing functions select_all find_text
...
menu_bar = Menu(root) # adding horizontal menu
# adding menu  item:File at the horizontal menu
...
# adding menu item:Edit at the horizontal menu
...
# adding menu item:View at the horizontal menu
...
# the color scheme is definet by using a dictionary
...
#adding menu item:About at the horizontal menu
...
root.config(menu=menu_bar) # final meniu orizontal
# adding icons bar for fast functions
shortcut_bar = Frame(root, height=25, background='light sea green')
shortcut_bar.pack(expand='no', fill='x')
# adding number bar on the left
line_number_bar = Text(root, width=4, padx=3, takefocus=0, border=0, background='khaki',state='normal', wrap='none')
line_number_bar.pack(side='left', fill='y')
# adding editing zone through Control Text
content_text = Text(root, wrap='word', undo=1)
content_text.pack(expand='yes', fill='both')
# adding a control for the Scrollbar and tie it to the editing control
scroll_bar = Scrollbar(content_text)
content_text.configure(yscrollcommand=scroll_bar.set)
scroll_bar.config(command=content_text.yview)
scroll_bar.pack(side='right', fill='y')
# adding a keybind for help
content_text.bind('<KeyPress-F1>', display_help_messagebox)
# adding keybinds for File (New, Open, Save, Save as)
content_text.bind('<Control-N>' or '<Control-n>', new_file)
content_text.bind('<Control-O>' or '<Control-o>', open_file)
content_text.bind('<Control-S>' or '<Control-s>', save)
# adding keybind Ctr+F(f)/Ctrl+A(a)/Ctrl+Y(y) for Edit menu
content_text.bind('<Control-f>' or '<Control-F>', find_text)
content_text.bind('<Control-A>' or '<Control-a>', select_all)
content_text.bind('<Control-y>' or '<Control-Y>', redo)
# overwrite the button close and redirecting it to exit_editor
